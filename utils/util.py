...
def save_history(
    cls_orig_acc, clease_trig_acc, cls_trig_loss, at_trig_loss, at_epoch_list, logs_dir
):
    dataframe = pd.DataFrame(
        {
            'epoch': at_epoch_list,
            'cls_orig_acc': cls_orig_acc,
            'clease_trig_acc': clease_trig_acc,
            'cls_trig_loss': cls_trig_loss,
            'at_trig_loss': at_trig_loss,
        }
    )
...
def plot_curve(clean_acc, bad_acc, epochs, dataset_name):
    ...
    plt.plot(
        np.arange(0, N), bad_acc, label="Attack Success Rate", marker='o', color='red'
    )
